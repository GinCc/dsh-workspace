/**
 * dsh-plugin-token-usage — host half.
 *
 ...
 * message `source`), so no adapter or token-meter coupling is needed.
 */
...
const ROUTE = '/plugins/token-usage'
const MAX_SESSIONS = 5000

/**
 * Fold one session log into a stats row.
 * @param {import('@deepseek-ai/dsh-session/types').SessionHeader} header - session metadata.
...
/**
 * Plugin body: register the HTTP route.
...
export function apply(ctx) {
  ...
  async function computeStats() {
    ...
    const sessions = []
    for (const snapshot of snapshots.slice(0, MAX_SESSIONS)) {
      const id = String(snapshot.header.id)
      let cached = cache.get(id)
      if (cached === undefined || cached.revision !== snapshot.revision) {
        try {
          // readFrom is the pure suffix read: no recovery commit, no live
          // coordinator state — safe for an observer that must never mutate.
